...
for x in results:
    ...
    sql = "SELECT MARKETID,SELECTIONID,RATING,RESULT,BACK FROM RESULTS WHERE MARKETID = {} ORDER BY RATING".format(x[0])
    #myprint (sql)
    cursor.execute(sql)
    ...
    for row in RatingResults:
        if (row[4] < favOdds):
            favOdds = row[4]
            favSelection = row[1]
        if (row[3] == "WINNER" or row[3] == "LOSER"):
            runners = runners + 1
        if (row[2] == 1000):
            sethalf = 1
        if (rating > row[2]):
    ...
    sql = "INSERT INTO SCORES(MARKETID, SELECTIONID, FORECAST, BACK, LAY, DATE, TIME, VENUE, NAME, FORM, RACE, RATING, RESULT, SCORE, RUNNERS, FAVOURITE) \
                VALUES ('%s', '%s', '%s', '%s', '%s' , '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') " % \
                    (x[0], x[1], x[3], x[10], x[11], x[6], x[7], x[8], x[2], x[4], x[5], x[9], x[12], str(score), str(runners), str(favourite))
    cursor.execute(sql)
