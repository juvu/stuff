...
def getMarketStatus(SSOID, market):
    ...
    for w in range(len(market['runners'])):
        ...
        dictItem = {"selectionID":selectionID, "back" : back, "lay" : lay}
            ...
            if (item['back'] <= BAILODDS):
                ...
                if (liability > 0.0):
                    ...
                    if (bet < 2.0):
                        bet = 2.0
                    betAmount = "{:.2f}".format(bet)
                    myprint ("BAILING Market {} Player {} odds {} bet {}".format(marketList[hrow]['marketId'],str(item['selectionID']),str(item['back']),betAmount))
